...
from samtools.sam_api.search_preprocessor import get_search_parameter, sanitize_input
...
class TestSearchPreprocessor:
    ...
    @staticmethod
    @pytest.mark.parametrize("resembles_ncage_code", 
    ['advex', 'adyen', 'aesub', 'alhdd', 'ampad', 'ansys', 'arcus',
     'atlas', 'ayden'])
    ...
    @staticmethod
    @pytest.mark.parametrize("not_ncage_code",
    ['adept', 'alpha', 'anker', 'apple', 'arata', 'astra',
       'avent', 'avery', 'aveva', 'avnet', 'aztek', 'baker',
       'bandh', 'beamq', 'beats', 'bestb', 'black', 'brady', 'braun',
       'bruel', 'cable', 'cerex', 'chase', 'clark', 'crane', 'crest',
       'cvent', 'cytec', 'dance', 'dataq', 'delta', 'dewey', 'dmark',
       'durex', 'dwyer', 'dynex', 'eagle', 'ebags', 'eckel', 'eflux',
       'eksma', 'elmet', 'emrax', 'endaq', 'enmet', 'erlab', 'ersad',
       'ettus', 'excel', 'extec', 'fabas', 'fastc', 'fedex', 'fetch',
       'fluke', 'fully', 'futek', 'genex', 'ghent', 'glass', 'grahl',
       'huber', 'human', 'hunts', 'jabra', 'jakar', 'jewel', 'kmart',
       'kpaul', 'laser', 'lemke', 'lprcs', 'madge', 'magna', 'maple',
       'meyer', 'nemal', 'neweg', 'nlyte', 'padua', 'parts', 'pater',
       'pchub', 'pcmag', 'prusa', 'quest', 'ratta', 'rcats', 'rexel',
       'sager', 'sampe', 'seals', 'sears', 'seeed', 'sepac', 'sharp',
       'shell', 'spark', 'stamp', 'steam', 'sweet', 'taber', 'talas',
       'tattu', 'tdarr', 'tesla', 'tnutz', 'trace', 'trane', 'uhaul',
       'ussfp', 'utmel', 'valve', 'veeam', 'versa', 'watec', 'watts',
       'wawak', 'zaber', 'zazzl', 'zemax', 'zubax', 'zuken'])
    ...
    def test_input_cage_code_with_whitespace(whitespace):
        _id = "12345"
        assert get_search_parameter(f"{whitespace}{_id}{whitespace}") == {"cageCode": _id}


    @staticmethod
    @pytest.mark.parametrize("uei_sam", ["123456789012", "ABCDEFGHJKL4", "12AB56789012"])
    ...
    @staticmethod
    @pytest.mark.parametrize("website", ["name.com", "www.name.com",
                            "name.org", "name.gov", "name.us"])
    def test_input_website(website):
        assert get_search_parameter(website) == \
            {'q': f'(*{website}*)'}


    @staticmethod
    def test_input_website_with_endpoint():
        assert get_search_parameter("name.com/name") == \
            {'q': '(*name.com*)'}
    ...
    def test_input_website_with_http(http):
        assert get_search_parameter(f"{http}www.name.com") == \
            {'q': '(*www.name.com*)'}
